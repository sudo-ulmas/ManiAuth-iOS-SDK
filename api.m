...
@implementation PullyBrightnessBox
- (instancetype)initWithValue:(PullyBrightness)value {
  self = [super init];
  if (self) {
    _value = value;
  }
...
@implementation CancelReasonBox
- (instancetype)initWithValue:(CancelReason)value {
  self = [super init];
  if (self) {
    _value = value;
  }
...
+ (instancetype)makeWithPaymentSystemId:(nullable NSString *)paymentSystemId
    ...
    lastLogin:(nullable NSString *)lastLogin
    ...
    brightness:(nullable PullyBrightnessBox *)brightness {
  ...
  pigeonResult.brightness = brightness;
  return pigeonResult;
}
+ (HostInfo *)fromList:(NSArray<id> *)list {
  HostInfo *pigeonResult = [[HostInfo alloc] init];
  ...
  pigeonResult.lastLogin = GetNullableObjectAtIndex(list, 4);
  pigeonResult.environment = GetNullableObjectAtIndex(list, 5);
  pigeonResult.residentType = GetNullableObjectAtIndex(list, 6);
  pigeonResult.designVariant = GetNullableObjectAtIndex(list, 7);
  pigeonResult.brightness = GetNullableObjectAtIndex(list, 8);
  return pigeonResult;
}
+ (nullable HostInfo *)nullableFromList:(NSArray<id> *)list {
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 132: {
      NSNumber *enumAsNumber = [self readValue];
      return enumAsNumber == nil ? nil : [[PullyBrightnessBox alloc] initWithValue:[enumAsNumber integerValue]];
    }
    case 133: {
      NSNumber *enumAsNumber = [self readValue];
      return enumAsNumber == nil ? nil : [[CancelReasonBox alloc] initWithValue:[enumAsNumber integerValue]];
    }
    case 134: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[ManiEnvironmentBox class]]) {
    ...
    [self writeByte:131];
    [self writeValue:(value == nil ? [NSNull null] : [NSNumber numberWithInteger:box.value])];
  } else if ([value isKindOfClass:[PullyBrightnessBox class]]) {
    PullyBrightnessBox *box = (PullyBrightnessBox *)value;
    [self writeByte:132];
    [self writeValue:(value == nil ? [NSNull null] : [NSNumber numberWithInteger:box.value])];
  } else if ([value isKindOfClass:[CancelReasonBox class]]) {
    CancelReasonBox *box = (CancelReasonBox *)value;
    [self writeByte:133];
    [self writeValue:(value == nil ? [NSNull null] : [NSNumber numberWithInteger:box.value])];
  } else if ([value isKindOfClass:[Token class]]) {
    [self writeByte:134];
  ...
  messageChannelSuffix = messageChannelSuffix.length > 0 ? [NSString stringWithFormat: @".%@", messageChannelSuffix] : @"";
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:nullGetApiCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(cancelReason:error:)], @"HostAppApi api (%@) doesn't respond to @selector(cancelReason:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        CancelReasonBox *boxedCancelReason = GetNullableObjectAtIndex(args, 0);
        CancelReason arg_reason = boxedCancelReason.value;
        FlutterError *error;
        [api cancelReason:arg_reason error:&error];
